/* TurboChat — export_core.js  |  document_idle  |  all platforms  v2.2
 *
 * KEY FIX v2.2:
 *   blob: URLs inherit the host site's CSP (chatgpt.com / claude.ai / gemini all
 *   block inline scripts). ALL <script> tags removed from the blob HTML.
 *   window.print() is now called from the content script directly on the
 *   window reference returned by window.open() — no CSP dependency at all.
 */
(function () {
  'use strict';
  ...
  /* ── XHR fetch for cross-origin images (Word-safe base64) ─────────────── */
  function fetchAsBase64(src) {
    return new Promise((resolve) => {
      try {
        ...
        xhr.timeout = 10000;
        // Include credentials for authenticated Google CDN images
        xhr.withCredentials = src.includes('google') || src.includes('googleusercontent');
        xhr.onload = () => {
  ...
  async function extractImages(container) {
    ...
    for (const img of container.querySelectorAll('img')) {
      ...
      // Dimension check — use layout rect as final fallback for lazy/hidden images
      ...
      // If all dimension sources are 0, assume image hasn't loaded — still try it
      // (XHR fetch below doesn't need dimensions). Only hard-skip definitively tiny images.
      ...
      if (!dataUrl || dataUrl === 'data:,') dataUrl = await fetchAsBase64(img.src);
  ...
  <div class="body">${m.html || escHtml(m.text)}${imgHtml}</div>
    ...
    /* No <script> block — window.print() is called from the content script
       on the win reference after the page loads.  CSP-safe on all platforms. */
...
<p style="margin-top:32pt;padding-top:12pt;border-top:1px solid #eee;
  ...
  function buildJson(msgs, { title, platform, aiLabel }) {
    return JSON.stringify({
      ...
      exported_by: 'TurboChat for All AI v2.2',
  ...
  /* ── openPrintPage — drives print() from content script, not blob HTML ── */
  function openPrintPage(html) {
    ...
    /* Call win.print() from HERE (content script) once the page has loaded.
       This works because the blob URL is same-origin with the page that created it,
       so we have full access to the window object — no CSP involved at all. */
    win.addEventListener('load', () => {
      setTimeout(() => {
        try { win.print(); } catch (e) { /* user closed window early — ignore */ }
      }, 600); // small delay lets fonts + images settle
    });

    /* Keep the blob URL alive for the entire print session (user may re-print).
       Revoke after 5 minutes — generous but not forever. */
