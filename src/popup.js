...
async function doExport(format, btn) {
  // All formats free — supported by Ko-fi ☕
...
// ── Prompt Library — open as Chrome side panel ────────────────────────────────
async function openPromptsPanel() {
  try {
    const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
    if (tab && chrome.sidePanel) {
      await chrome.sidePanel.setOptions({ tabId: tab.id, path: 'prompts.html', enabled: true });
      await chrome.sidePanel.open({ windowId: tab.windowId });
      window.close();
    } else {
      // Fallback: open as floating popup window on right side
    ...
    console.warn('[TurboChat] Side panel error:', err);
    chrome.tabs.create({ url: chrome.runtime.getURL('prompts.html') });
    window.close();
  }
}

document.getElementById('prompts-card')?.addEventListener('click', openPromptsPanel);
document.getElementById('prompts-card')?.addEventListener('keydown', e => {
  if (e.key === 'Enter' || e.key === ' ') openPromptsPanel();
});

// ── Ko-fi support button ──────────────────────────────────────────────────────
document.getElementById('btn-kofi')?.addEventListener('click', () => {
  chrome.tabs.create({ url: 'https://ko-fi.com/codylim' });
  window.close();
});
